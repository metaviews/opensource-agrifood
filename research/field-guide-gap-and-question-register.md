...
2. G-OSA-22 residual (next-cycle candidates, not yet run): verify Priva/Hoogendoorn/Signify open programmes at source (the "open architecture" claim is contested, not closed); cover open vertical-farm / mushroom CEA separately; establish whether any open greenhouse data standard is implemented in production (CGO adoption gap); establish whether Mycodo scales beyond small/maker.
