# Open Agrifood Examples Catalogue

Research date: 2026-08-01

This catalogue is the browseable index for the curated examples collection. It is intentionally selective. The collection has no predetermined final count. Expansion should respond to analytical, regional, governance, or non-AI coverage gaps.

## Collection map

| Example | Region / reach | Open layer | Agrifood role | Status |
|---|---|---|---|---|
| [Open Ag Data Alliance](records/oada.md) | International / precision agriculture | code, API, interoperability | agricultural data exchange | curated |
| [JoinData](records/joindata.md) | Netherlands | data permissions, governance | farmer-controlled agricultural data | curated |
| [Mozilla Common Voice](records/common-voice.md) | Global / multilingual | community datasets | upstream language infrastructure | curated |
| [Kallaama](records/kallaama.md) | Senegal / West Africa | open agricultural speech dataset (Wolof, Pulaar, Sereer; CC BY 4.0) | agricultural speech corpora for ASR | candidate |
| [Open Source Seed Initiative](records/open-source-seed-initiative.md) | Global | seed pledge, biological commons | open breeding and seed sovereignty | curated |
| [GODAN](records/godan.md) | Global / Africa-relevant | open-data coordination | agricultural and nutrition data ecosystem | curated |
| [Open Data Kit](records/open-data-kit.md) | Global / African deployments | software and field infrastructure | agricultural research and data collection | curated |
| [CGIAR / AgriLLM](records/cgiar-agrillm.md) | Global South / multilateral | model artifact, public-interest AI | agricultural advisory and research AI | candidate |
| [Bioleft](records/bioleft.md) | Argentina / Latin America | seed commons, participatory data | open breeding and seed exchange | curated |
| [farmOS](records/farmos.md) | International | software, farm records | farm management and planning | curated |
| [LiteFarm](records/litefarm.md) | Canada-connected (UBC) / international | GPL-3.0 farm-management software, API, participatory design | diversified and agroecological farm management | curated |
| [Ekylibre](records/ekylibre.md) | France-origin / global | open-source FMIS (AGPL-3.0) | farm management software | curated |
| [ROMI](records/romi.md) | Europe / microfarms | hardware, software, robotics | weeding, monitoring, phenotyping | curated |
| [Digital Earth Africa crop-type mapping](records/digital-earth-africa-crop-type.md) | Africa | geospatial workflow, notebooks | crop mapping and analytics | curated |
| [Ag Data Commons](records/ag-data-commons.md) | United States / public research | data catalogue, repository, FAIR infrastructure | agricultural research-data access | curated |
| [Holos](records/holos.md) | Canada / federal public research | open model and software | whole-farm GHG modelling | candidate |
| [Open Food Network Canada](records/open-food-network-canada.md) | Canada / local food | software commons, marketplace | local food commerce and community hubs | curated |
| [Agri-food Data Canada](records/agrifood-data-canada.md) | Canada / research ecosystem | FAIR data, schemas, portals | agricultural research-data infrastructure | candidate |
| [Standard Data Platform for Autonomous Agriculture](records/standard-data-platform-autonomous-agriculture.md) | Canada / commercial-public consortium | claimed data commons, digital twin | autonomous farm planning | lead |
| [UBC Farm Seed Catalogue / Seed Hub](records/ubc-farm-seed-catalogue.md) | Canada / British Columbia | public-domain seed, seed research, cooperative infrastructure | regional seed production and sovereignty | candidate |
| [Sovereign Seeds](records/sovereign-seeds.md) | Canada / Indigenous-led | seed stewardship, cultural food sovereignty | Indigenous seed and agricultural food sovereignty | lead |
| [Brian Tischler](records/brian-tischler.md) | Canada / Alberta | open software, open hardware, farmer-led community | precision agriculture and autonomous machinery | curated |
| [Matt Reimer](records/matt-reimer.md) | Canada / Manitoba | open hardware, public source code, maker community | autonomous grain cart and farm robotics | candidate |
| [L'Atelier Paysan](records/latelier-paysan.md) | France / Europe | machinery designs, self-build training, cooperative governance | small-scale and agroecological farm equipment | curated |
| [FarmBot](records/farmbot.md) | US / global | CNC robot hardware, software (MIT), benefit-corporation openness | small-scale automated food production | curated |
| [OS Farm](records/osfarm.md) | France / EU | open agritech interoperability standard (France 2030) | farm equipment interoperability consortium | candidate |
| [Open Source Ecology](records/open-source-ecology.md) | US / global | machine design commons (GPL / CC-BY-SA) | open-source tractor and farm machinery set | curated |
| [ArduPilot](records/ardupilot.md) | Global | autopilot firmware (GPL-3.0), community infrastructure | agricultural drones and robotics enabler | curated |
|| [Vinduino](records/vinduino.md) | US / California | sensing and irrigation hardware designs (GPL-3.0) | vineyard irrigation management | curated |
|| [Compost for Good](records/compost-for-good.md) | US / NY Adirondack | free open-source design guide + operating manual (licence-formality unverified) | community-scale food-waste composting, rotating drum | curated |
|| [Kon-Tiki kiln](records/kon-tiki-kiln.md) | Switzerland / global via open transfer | open-source biochar kiln by publication + practice (licence-formality thin) | farmer-scale biochar production | curated |
|| [IBI open-source biochar](records/ibi-open-source-biochar.md) | Global / IBI aggregator | open-access collection of multiple kiln designs (mixed provenance) | small-scale biochar production designs aggregator | curated |
|| [compost-sensor](records/compost-sensor.md) | Hobby / global via GitHub | Arduino+LoRa compost temp sensor (MIT) | hobby compost pile monitoring | candidate |
|| [Smart Worm Habitat](records/smart-worm-habitat.md) | Hobby / global via GitHub | GPL-3.0 continuous-flow worm bin concept (unbuilt, dormant) | vermicompost system design concept | candidate |
|| [Farm Hack](records/farm-hack.md) | US / UK | design-sharing platform and events | farm tool commons infrastructure | candidate |
| [ISOBlue](records/isoblue.md) | US / Purdue | open telematics hardware and software (MIT family) | machine-data access and precision farming telematics | curated |
| [AgIsoStack++](records/agisostack.md) | Global | open ISOBUS standard implementation (MIT / GPL family) | farm machinery interoperability and data | curated |
| [Think Oggún](records/oggun.md) | Cuba-facing / US-made, 4-country distributors | open-manufacturing transfer model (licence-fee) | low-cost smallholder tractor | curated |
| [Flying Labs Network](records/flying-labs.md) | ~40 countries, Global South | localized drone/data/AI capacity network | agricultural drone and data services | curated |
| [AfricaOSH](records/africaosh.md) | Africa | open-science-hardware community infrastructure | continental open hardware substrate | candidate |
| [Open Agroecology Lab (REGOSH)](records/regosh-open-agroecology-lab.md) | Argentina | open farm-data stack + open sensing collaborations | agroecological knowledge and data infrastructure | curated |
| [AgriStack / Digital Agriculture Mission](records/agristack.md) | India | consent architecture, registries, UFSI API | national farmer-data stack | curated |
| [WAGRI](records/wagri.md) | Japan | open API platform (membership, partly charged) | national agricultural data collaboration | curated |
| [Bhashini](records/bhashini.md) | India / national | language DPI; PM-KISAN voice chatbot (11 languages) | state language infrastructure (MeitY) | curated |
| [CEADS / AgriDataSpace / FSDN](records/ceads.md) | European Union | data-space standards and federated governance | supranational agricultural data space | curated |
| [ONDC / Beckn](records/ondc.md) | India / global | open protocol and specifications | open digital commerce incl. agriculture | curated |
| [FarmStack](records/farmstack.md) | Global (Digital Green) | Apache-2.0 data-exchange DPG | agricultural data-exchange infrastructure | curated |
| [FarmerChat](records/farmerchat.md) | India, Kenya, Nigeria, Ethiopia, Brazil | open foundation (Apache-2.0 code, CC-BY-4.0 data) | deployed AI farm advisory (1.6M farmers) | curated |
| [Sinong (Shennong)](records/sinong.md) | China / global distribution | open weights 8B/32B; proprietary corpus | agricultural foundation model (state university) | curated |
| [Alibaba ET Agricultural Brain](records/et-agricultural-brain.md) | China | none (closed platform contrast) | agricultural AI platform (company) | candidate |
| [National Agricultural Scientific Data Infrastructure](records/china-ag-data-infrastructure.md) | China | open research-data portals and standards | national scientific data centers | candidate |
| [All Farmers Code (全农码)](records/quannongma-all-farmers-code.md) | China / national | none verified (state-private digital identity platform) | MARA Big Data Development Center + private operator | candidate |
| [Appropedia / OSAT](records/appropedia-osat.md) | Global (developing-world oriented) | CC BY-SA open design commons | appropriate-technology wiki + academic current | curated |
| [Farm Africa / NECPA Solar Dryer](records/farm-africa-necpa-solar-dryer.md) | Uganda | openly published design document (licence unstated) | NGO + producer association | candidate |
| [NAPDC / Ag Data Coalition](records/napdc-ag-data-coalition.md) | United States | grower-controlled data governance and storage | grower cooperative + university-led coalition | curated |
| [Agdatahub](records/agdatahub.md) | France / EU | DGA consent intermediation; API-Agro standard | sector/institutional company | curated |
| [Farmers Business Network](records/fbn.md) | United States | none (contested "farmer-owned" framing) | venture-backed company | candidate |
| [SAOS](records/saos.md) | Scotland / UK | cooperative data governance; federation-owned traceability infrastructure (ScotEID) | cooperative federation (est. 1905) + subsidiaries | curated |
| [Grower Information Services Cooperative](records/gisc.md) | United States | grower-owned data aggregation; AgXchange lineage | grower cooperative → GADC → NAPDC | candidate |
| [OpenSourceSeeds / OSSL](records/opensourceseeds-ossl.md) | Germany / Europe | contractual open seed licence (derivatives bound) | seed commons licensing | curated |
| [Potato Park](records/potato-park.md) | Peru / Quechua | Indigenous biocultural commons | potato genetic commons | curated |
| [SEARICE](records/searice.md) | Southeast Asia | farmer-led seed systems and community seed banks | regional seed commons | curated |
| [Navdanya](records/navdanya.md) | India / Bhutan | movement-managed community seed banks | seed sovereignty movement | candidate |
| [Mycodo](records/mycodo.md) | United States / international | open-source greenhouse & hydroponic control software (GPL-3.0) | maintainer-led (Kyle Gabriel) | curated |
| [MIT OpenAg Food Computer](records/openag-food-computer.md) | United States (MIT Media Lab) | claimed all-open CEA; reality archived (contested) | MIT Media Lab (defunct) | candidate |
| [Common Greenhouse Ontology](records/common-greenhouse-ontology.md) | Netherlands / EU (TNO) | open greenhouse data-interoperability standard (Apache-2.0) | research institute (TNO) | candidate |
| [Wageningen Autonomous Greenhouse Challenge datasets](records/wageningen-agc-datasets.md) | Netherlands (WUR) | open CEA cultivation datasets (DOI-published) | university research (WUR) | candidate |
| [OpenSprinkler](records/opensprinkler.md) | US / Global | firmware (GPL-3.0), hardware (CERN-OHL-S-2.0) | open-source irrigation automation controller | curated |
| [AquaCrop-OSPy](records/aquacrop-ospy.md) | International (Univ. of Manchester / FAO) | crop-water simulation engine (Apache-2.0) | scientific crop-water & yield modeling | curated |
| [OpenFence](records/openfence.md) | Australia / Global | firmware + hardware concepts (GPL-3.0, dormant) | open GPS digital virtual livestock fencing | candidate |
| [Open Source Range (OSR) GPS Collar](records/osr-gps-collar.md) | US / Global | low-cost GPS telemetry collar designs (open science) | rangeland livestock tracking & grazing ecology | candidate |
| [Real Seeds DIY Seed Cleaner](records/real-seeds-cleaner.md) | UK / Global | open hardware design & manual (public commons) | post-harvest smallholder seed winnowing & processing | curated |
| [ColdSnap / DIY CoolBot](records/coldsnap-coolbot.md) | Global | firmware & schematics (MIT / GPL-3.0 via Mycodo) | low-cost walk-in cold storage automation | candidate |
| [CoopCycle](records/coopcycle.md) | France / Global (70+ co-ops) | web & mobile logistics platform (Coopyleft) | worker-owned last-mile food & meal delivery logistics | curated |
| [Open Foris / SEPAL](records/open-foris-sepal.md) | International (UN FAO) | cloud geospatial & field survey suite (MIT) | agroforestry canopy, land-use monitoring & MRV | curated |
| [Our Sci Reflectometer & Bionutrient Meter](records/our-sci-reflectometer.md) | US / Global | handheld optical spectrometer & calibration data (GPL-3.0) | soil organic carbon & crop nutrient density testing | curated |
| [OpenFlexure Microscope](records/openflexure-microscope.md) | UK / Global (Tanzania, LatAm) | 3D-printed digital robotic microscope (CERN-OHL / GPL-3.0) | decentralized soil biology & crop pathology diagnostics | curated |
| [农业岛 / HUIZHI-nongyeOS-cloud](records/nongyedao-huizhi.md) | China | open-source agricultural IoT platform (MIT) | greenhouse, sensor & high-standard farmland management | candidate |
| [AgriMa & AgriAgent (后稷 / 稷丰)](records/agrima-shanxi.md) | China (Shanxi Ag Univ) | open-weight agricultural LLM & multimodal model (academic) | agronomic Q&A and multimodal disease diagnostics | candidate |
| [AgGateway Traceability API](records/aggateway-traceability-api.md) | US / international | public grain traceability API/specification (custom licence) | commingled grain traceability and interoperability | candidate |
| [Agricultural Commodity Exchange for Africa (ACE)](records/ace-africa.md) | Malawi / Southern and Eastern Africa | market information, warehouse receipts, electronic trade | structured grain marketing and farmer market access | candidate |
| [Community seed banks and CSBAN](records/community-seed-banks-nepal.md) | Nepal | farmer-managed biological commons, seed exchange, local-variety catalogue | seed sovereignty, agro-biodiversity, and farmer rights | candidate |
| [DESIS 2.0](records/desis-nepal.md) | Nepal | public-interest digital seed infrastructure; openness unresolved | seed demand, certification, tracking, and coordination | candidate |
| [WeClock](records/weclock.md) | International (UNI Global Union) | GPL-3.0 worker self-tracking app; worker-held data by design | worker-side labour-governance instrument (no agrifood deployment) | curated |
| [Digger's Mirth Collective Farm](records/diggers-mirth.md) | Burlington VT, USA | none (governance-open: worker collective, equal wage) | farmworker-cooperative ownership model | curated |
|| [SolarFood solar-drying project](records/solarfood-nepal.md) | Nepal / Bhutan | local-manufacturing and cooperative open-transfer lead | post-harvest preservation and women-led value capture | lead |
|| [Foodsoft](records/foodsoft.md) | International (DACH, NL) | co-op operating software (AGPL-3.0-or-later), Co-op Cloud recipe, Democratic Tech Fund co-funding circle | member-work food co-op operations | curated |
|| [FoodCoopShop](records/foodcoopshop.md) | Austria / DACH | producer-direct ordering platform (AGPL-3.0), single-maintainer open core + paid services | food co-ops and farm shops | curated |
|| [Foodsharing.de](records/foodsharing.md) | Germany/Austria/Switzerland | food-rescue coordination platform (AGPL-3.0) | peer-to-peer and business food rescue movement | curated |
|| [CSA K100:26](records/csa-k100-26.md) | Canada | national FLW terminology/measurement standard (paywalled) | food-loss-and-waste measurement language | candidate |
|| [IGFC mobile slaughter unit](records/igfc-mobile-slaughter.md) | US (Washington State) / model cited in Canada | cooperative governance of mobile slaughter infrastructure (no open-licensed artifacts) | small-producer slaughter access | curated |
|| [OpenEPCIS epcis-repository-ce](records/openepcis.md) | International (Germany-maintained) | EPCIS 2.0 event repository + tooling (Apache-2.0 community edition, open-core) | supply-chain traceability data layer | curated |
|| [Underground Meats open-source HACCP plan](records/underground-meats-open-haccp.md) | United States (Madison, WI) | CC0-validated HACCP plan (2013); host dead, survives in Wayback | shared food-safety compliance records | archived |
|| [Grain Bikes](records/grain-bikes.md) | US Northeast / global plans | bicycle-powered thresher, fanning mill, dehuller/flour mill build plans (CC BY 4.0) | micro-scale grain processing hardware | curated |
|| [ICAR Animal Data Exchange (ADE)](records/icar-ade.md) | International (10+ countries; Lactanet Canada on working group) | open JSON/OpenAPI livestock data standard (Apache-2.0) | milk-recording and animal data exchange | curated |
|| [Canadian Grain Commission grading data](records/cgc-grading-data.md) | Canada / federal | public grading rules and annual quality data (PDF/Excel, no schema; exports dataset OGL-Canada) | grain grading authority and quality data | curated |
|| [JBS ransomware attack 2021](records/jbs-ransomware-2021.md) | Multinational (Canada: Brooks AB) | none — REvil ransomware; US$11M ransom paid; national kill capacity idled | concentration as cyber vulnerability | curated |
|| [Sobeys/Empire ransomware attack 2022](records/sobeys-ransomware-2022.md) | Canada (national) | none — closed cloud retail stack; pharmacies down 4 days; $25M net of insurance | retail concentration as single point of failure | curated |
|| [Maple Leaf Foods ransomware attack 2022](records/maple-leaf-ransomware-2022.md) | Canada (national) | none — refused Black Basta ransom; plants ran on manual workarounds; ≥CA$23M | purchased vs structural resilience | curated |
|| [NEW Cooperative ransomware attack 2021](records/new-cooperative-ransomware-2021.md) | United States (Iowa) | none — BlackMatter hit a member-owned co-op; paper-ticket fallback; soil-mapping source code leak threat | co-op ownership without open assets under attack | curated |
|| [ISOBlue 2.0](records/isoblue.md) | United States (Purdue/AFIN) | MIT hardware/software telematics capture; season-long local storage, opportunistic sync | local operability as a resilience property | curated |
|| [CSA T201 food value chain cybersecurity standard](records/csa-t201-food-cybersecurity.md) | Canada (national standard, in development) | closed standard instrument tailoring IEC 62443 to agri-food; comment period closed 2026-09-09 | the policy surface where open could enter food cybersecurity | curated |
|| [SIKU](records/siku.md) | Arctic-wide (Canada, Alaska, Greenland) | free platform, no licence taken on user data — "You own your data. Nobody else does – not even SIKU" (ToS verified 2026-09-09); app code licence unverified | Indigenous data sovereignty as platform design | curated |
|| [NWMB Community-Based Monitoring Network](records/nwmb-cbmn.md) | Nunavut, Canada | harvester-owned data (non-exclusive, royalty-free licence to NWMB; verified from FAQ + Arctic Science 2020); 7,225 harvest records, 85 harvesters, 7 communities | community-owned monitoring feeding co-management | curated |
|| [SMART conservation software](records/smart-conservation.md) | Global (WWF/WCS/ZSL partnership) | open-source by project claim (licence text unverified — no public repo located); runs Nunavut CBMN | Global-South open source as northern food-governance data layer | curated |
|| [CyberTracker](records/cybertracker.md) | Global (Cape Town, South Africa origin) | MIT mobile client (GitHub API verified 2026-09-09); hosting now tiered with free lane for Indigenous communities | oldest Indigenous-knowledge software commons; maintenance-funding transition | curated |
|| [CORE-POS](records/core-pos.md) | Global / community-maintained | GPL-2.0 (GitHub API verified 2026-09-09; active); self-hosted co-op grocery POS | the open lane-level retail stack | curated |
|| [Arctic Co-operatives Limited](records/arctic-cooperatives.md) | Nunavut, NWT, Yukon (30+ member co-ops) | none at systems level — governance record; margins stable/declining 2019-23 while NWC profits rose (Li, via Food Secure Canada) | ownership vs systems openness in northern retail | curated |
|| [Harvest Food Hub](records/harvest-food-hub.md) | Northern Ontario (Algoma), expanding | UNVERIFIED — no public repo/licence found; built by SSMIC on OTF funding for Harvest Algoma | publicly funded northern food software: open release or Second Harvest pattern? | candidate |
|| [Global Fishing Watch](records/global-fishing-watch.md) | Global (US 501(c)(3)) | derived fishing-effort datasets (CC-BY-NC-4.0), open-source clients (Apache-2.0/GPL-3.0), public APIs (non-commercial); raw AIS closed | fishing transparency and IUU detection | curated |
|| [sdmTMB](records/sdmtmb.md) | Canada-origin (DFO Pacific Biological Station) / international | spatiotemporal GLMM R package (GPL-3, JSS-published) | fisheries stock-assessment science | curated |
|| [Stock Synthesis 3 (SS3)](records/ss3.md) | US (NOAA) / international incl. DFO Pacific | age-structured assessment engine (CC0-1.0) | fisheries stock assessment | curated |
|| [OpenCTD](records/openctd.md) | US-origin / international workshops | low-cost CTD oceanographic instrument (MIT) | coastal water-quality and marine sensing | curated |
|| [Monitorfish](records/monitorfish.md) | France / open source internationally | national fisheries monitoring & control platform (AGPL-3.0) | state fisheries enforcement infrastructure | curated |
|| [DFO aquaculture/sea-lice reporting](records/dfo-aquaculture-reporting.md) | Canada (BC site-level; national aggregate) | open licence-condition reporting datasets (OGL-Canada, CSV) | salmon-farming environmental transparency | curated |
|| [Skipper Otto CSF](records/skipper-otto.md) | Canada (BC; national membership) | none — B Corp-certified private CSF (45+ fishing families; transparent narrative, no open assets) | community-supported fishery market model | curated |
|| [BC Land Owner Transparency Registry](records/bc-lotr.md) | Canada (British Columbia) | public beneficial-ownership registry (name/PID search, free since 2024; not bulk) | land ownership transparency | curated |
|| [Terre de Liens](records/terre-de-liens.md) | France | none — citizen-shareholder farmland commons (449 farms; all-rights-reserved site) | farmland commons ownership model | curated |
|| [Land Matrix](records/land-matrix.md) | Global (low/middle-income focus) | open land-acquisition database (CC BY 4.0, CSV/API) | land-deal transparency | curated |
|| [fiboa & EuroCrops](records/fiboa-eurocrops.md) | International (30+ countries; 16 EU) | field-boundary spec (Apache-2.0) + parcel/crop dataset (CC BY-SA 4.0); no Canadian dataset | agricultural field-boundary geodata | curated |
|| [CanSIS / National Soil Database](records/cansis.md) | Canada / federal | soil landscapes, detailed surveys, 100m grids (OGL-Canada, FGDB/GPKG/GeoTIF) | soil and land-capability data | curated |
|| [HM Land Registry open data](records/hm-land-registry.md) | England & Wales | Price Paid Data (OGL v3.0, monthly since 2014) + 289 GitHub repos (192 MIT) | land transaction and registry data | curated |
|| [Framework Agreement land codes](records/framework-agreement-land-codes.md) | Canada (224 signatory First Nations) | none — community land codes, knowledge-sharing-only repository | Indigenous land jurisdiction | curated |

## Coverage at a glance

| Layer | Current examples |
|---|---|
| Open software | OADA, ODK, farmOS |
| Open models / weights | CGIAR / AgriLLM |
| Open datasets / language infrastructure | Common Voice, GODAN |
| Open agricultural speech datasets | Kallaama |
| Open standards / interoperability | OADA |
| Farmer or community data governance | JoinData |
| Biological commons | OSSI |
| Multilateral public-interest infrastructure | GODAN, CGIAR / AgriLLM |
| Open hardware / robotics | ROMI, FarmBot, ArduPilot (enabler) |
| Open farm machinery / self-build | L'Atelier Paysan |
| Open sensing / irrigation | Vinduino, OpenSprinkler (adjacent) |
| Machine design commons | Open Source Ecology |
| Farmer tool design-sharing network | Farm Hack |
| Open machine-data telematics | ISOBlue |
| Open ISOBUS implementation | AgIsoStack++ / Open-Agriculture |
| Open-manufacturing transfer model | Think Oggún |
| Open agritech interoperability standard (France 2030) | OS Farm |
| Global South drone/data capacity network | Flying Labs |
| Africa open-science-hardware community | AfricaOSH |
| Open agroecology stack (LatAm) | REGOSH Open Agroecology Lab |
| National DPI stack (consent architecture) | AgriStack |
| Language DPI (India) | Bhashini |
| Government ag data API platform | WAGRI |
| Supranational ag data space | CEADS / AgriDataSpace / FSDN |
| Open-protocol commerce network | ONDC / Beckn |
| Ag data-exchange DPG | FarmStack |
| Deployed open-data ag advisory AI | FarmerChat |
| Open-weight ag foundation model (China) | Sinong |
| Closed ag AI platform (China, contrast) | Alibaba ET Agricultural Brain |
| National ag digital identity platform (China) | All Farmers Code / 全农码 |
| National ag scientific data infrastructure (China) | National Agricultural Scientific Data Centers |
| Open design commons (appropriate technology) | Appropedia / OSAT |
| NGO-published open design (Uganda) | Farm Africa / NECPA solar dryer |
| Grower-controlled data cooperative (US) | NAPDC / Ag Data Coalition |
| Cooperative data federation (Scotland) | SAOS / ScotEID |
| Grower-owned data aggregation co-op (US) | GISC / AgXchange lineage |
| DGA data intermediation (France) | Agdatahub |
| Contested "farmer-owned" platform | Farmers Business Network |
| Open greenhouse/hydroponic control software | Mycodo |
| Open CEA data standard (unadopted) | Common Greenhouse Ontology |
| Open CEA cultivation datasets | Wageningen Autonomous Greenhouse Challenge |
| Failed "all-open" CEA claim (cautionary) | MIT OpenAg Food Computer |
| Contractual open seed licence | OpenSourceSeeds / OSSL |
| Indigenous biocultural commons | Potato Park |
| Farmer-led seed systems (SE Asia) | SEARICE |
| Seed sovereignty movement (India) | Navdanya |
| Open farm management | farmOS |
| Open-source farm management (France/global) | Ekylibre |
| African geospatial infrastructure | Digital Earth Africa crop-type mapping |
| Public agricultural data repository | Ag Data Commons |
| Latin American biological commons | Bioleft |
| Canadian public agricultural model | Holos |
| Canadian local-food software commons | Open Food Network Canada |
| Canadian FAIR agricultural data infrastructure | Agri-food Data Canada |
| Canadian contested data-commons case | Standard Data Platform for Autonomous Agriculture |
| Canadian regional seed infrastructure | UBC Farm Seed Catalogue / Seed Hub |
| Canadian Indigenous seed-sovereignty lead | Sovereign Seeds |
| Canadian farmer-led open-source founder | Brian Tischler |
| Canadian farmer-maker and open robotics case | Matt Reimer |
| Clearly farmer-, Indigenous-, or community-governed | JoinData; OSSI is adjacent but not equivalent; Tischler and Reimer are farmer-led but not shown to govern their wider projects |
| Non-AI examples | OADA, JoinData, Common Voice, OSSI, GODAN, ODK, Bioleft, farmOS, ROMI, Digital Earth Africa, Ag Data Commons |
| Open irrigation controller & firmware | OpenSprinkler |
| Open crop-water modeling engine | AquaCrop-OSPy |
| Open livestock virtual fencing prototype | OpenFence |
| Open livestock GPS telemetry collar | Open Source Range (OSR) GPS Collar |
| Open post-harvest seed cleaner & aspirator | Real Seeds DIY Seed Cleaner |
| Open cold-storage controller & firmware | ColdSnap / DIY CoolBot (Mycodo) |
| Worker-cooperative food delivery & logistics | CoopCycle |
| Open agroforestry canopy & forest monitoring | Open Foris / SEPAL |
| Open field spectroscopy & soil carbon estimation | Our Sci Reflectometer |
| Open robotic microscope & soil pathology | OpenFlexure Microscope |
| Grain traceability API / specification | AgGateway Traceability API |
| Grain market information & warehouse receipts | ACE Africa |
| Chinese open agricultural IoT / SI platform | 农业岛 / HUIZHI-nongyeOS-cloud |
| Chinese open-weight agricultural domain LLM | AgriMa & AgriAgent |
| Open bioreactor platform (fermentation valorization) | Pioreactor |
| Surplus rescue platform (open claim, no licence) | Sharing Excess |
| Closed/monetized rescue app (Canada) | Second Harvest Food Rescue App |
| B Corp surplus marketplace on public contracts (Canada) | FoodMesh / Mesh Exchange |
| Certification capture of upcycled definition | Upcycled Certified (UFA/WFCF) |
| Closed Canadian upcycling flagship | LOOP Mission |

This table is a current map, not a claim that these layers are equally represented geographically or institutionally.

## Curation notes

The collection currently contains a deliberate majority of non-AI cases. That is a feature: the project’s AI emphasis should be tested against the wider ecology of open agrifood rather than allowed to define it in advance.

The open-hardware/robotics/sensing cell was expanded on 2026-08-02 from a single record (ROMI) to a full layer: farmer cooperative self-build (L'Atelier Paysan), company-backed open hardware (FarmBot), machine design commons (Open Source Ecology), open autopilot infrastructure (ArduPilot), grower-led sensing (Vinduino), and peer design-sharing (Farm Hack). A second pass the same day added the machine-data layer beneath the machines: open telematics (ISOBlue) and an open ISOBUS implementation (AgIsoStack++), alongside the July 2026 FTC/Deere right-to-repair settlement as the regulatory context. A third pass targeted the Global South gap (G-OSA-15): the collection now includes the open-manufacturing transfer model of Think Oggún (Cuba-facing), the Flying Labs Network (drone/data/AI capacity across ~40 countries), AfricaOSH (continental open-science-hardware community), and the REGOSH Open Agroecology Lab (Argentina). A fourth pass added the DPI-for-agriculture layer: AgriStack (India), WAGRI (Japan), CEADS/AgriDataSpace/FSDN (EU), ONDC/Beckn (open-protocol commerce), and FarmStack (data-exchange DPG). A fifth pass covered China's agricultural digital infrastructure: Sinong (open-weight foundation model), Alibaba ET Agricultural Brain (closed platform contrast), and the national scientific data infrastructure. A sixth pass targeted the farm-level Global South hardware gap (G-GS-01), resolving it into three shapes: open design commons (Appropedia/OSAT), NGO-published open designs (Farm Africa/NECPA solar dryer), and affordable-but-closed commercial hardware; the farmer-led, explicitly licensed layer remains confirmed absent. A seventh pass covered data cooperatives and farmer data governance beyond the Netherlands: NAPDC/Ag Data Coalition (US grower cooperative), Agdatahub (France, DGA intermediation), and Farmers Business Network (contested "farmer-owned" venture platform). An eighth pass expanded the biological-commons layer beyond OSSI/Bioleft: OpenSourceSeeds/OSSL (contractual seed licence), Potato Park (Indigenous biocultural commons), SEARICE (farmer-led seed systems), and Navdanya (seed sovereignty movement). The remaining thin cell at that point was the maintenance/funding profiles (G-OSA-08), addressed the same day by a comparative verification pass across all 49 records (research/2026-08-maintenance-funding-profiles.md).

A cross-cell landscape synthesis was added on 2026-08-03 (research/2026-08-landscape-synthesis.md): the nine discovery cycles converted into seven cross-cutting currents, the contested-claims ledger (C-OSA-01..04) re-scored with provisional verdicts, and the gap queue re-ranked by what would change the synthesis most. The collection itself was unchanged by the synthesis pass. On 2026-08-06 a new public-source cycle opened and resolved the greenhouse/CEA automation cell (G-OSA-22, research/2026-08-greenhouse-cea-open-automation-scan.md): the open CEA layer exists as three partial shapes — open control software at small/maker scale (Mycodo, GPL-3.0, curated), open cultivation data (Wageningen Autonomous Greenhouse Challenge datasets, DOI-published ×4 editions, candidate), and an open but unadopted data standard (Common Greenhouse Ontology, Apache-2.0, candidate) — while the flagship "all-open" claim (MIT OpenAg Food Computer) is archived, licence-unresolved, and discredited (candidate, contested). No open layer reaches Priva/Hoogendoorn-scale commercial control; the openness thesis holds for data and hobby control, fails at commercial deployment and data-format interoperability. The same day, the OADA record (the queue's first item, hinge for C-OSA-01) was verified in depth: active Purdue OATS-maintained reference server (commits 2026-08-01) against an absent adoption layer — no verified OADA-compliant implementations or farmer-facing deployments, with ADC/NAPDC explicitly disclaiming OADA as its storage layer (research/2026-08-oada-implementation-scan.md). The G-OSA-20 follow-up then verified the data-cooperative durability leads: SAOS (Scotland, curated) — a 120-year-old federation with durable, government-contracted, subsidiary-owned traceability infrastructure (ScotEID, 2008-2026) but deferred farmer share-ownership and a feasibility-stage farmer-owned platform; and GISC (US, candidate) — the "only grower-owned data co-op" claim now unverifiable behind a maintenance-mode site, with its lineage folded into GADC/AgXchange/NAPDC (research/2026-08-data-coop-durability-followup.md). Q-OSA-07 stays answered in the negative: no data cooperative outside the Netherlands at verified farmer-controlled scale. A five-language verification pass (research/2026-08-local-language-verification-pass.md) then closed the local-language residual of G-OSA-15 (the Global South absence is robust across Spanish, Portuguese, French, Hindi, and Chinese — refined into research-led, social-technology, and maker-media shapes) and partially resolved G-OSA-19 from Chinese primary sources: the Action Plan's dated milestones, the All Farmers Code launched 2023 with a private co-developer/operator (chaojima), and the Zhejiang pilot. New records from that pass: Ekylibre (curated, active AGPL FMIS), OS Farm (candidate, France 2030 open agritech consortium), and All Farmers Code (candidate, state-private platform). The G-OSA-03 Common Voice pass (research/2026-08-common-voice-agrifood-applications.md) then resolved the language-data gap into shapes: Common Voice itself has no farmer-facing deployment (verified uses are a CIMMYT/Farm Radio International research chain via XLS-R and Mozilla's Kiswahili program-funded Haki des Femmes pilot), while the wider open-language-data layer shows the strongest deployment evidence in the open-AI stack — FarmerChat (1.6M farmers, five countries, Apache-2.0 code, CC-BY-4.0 data), Bhashini's PM-KISAN voice chatbot (11 languages), Ama KrushAI, and the Kallaama open agricultural speech dataset. New records: FarmerChat and Bhashini (curated), Kallaama (candidate); C-OSA-03 split with the language layer partially evidenced. The landscape synthesis was updated to v2 the same day (research/2026-08-landscape-synthesis-v2.md supersedes v1): C-OSA-01 hardened to contested-evidenced, C-OSA-03 split, Q-OSA-07 answered in the negative, and a language/speech column added to the cross-cell matrix.

The next additions should be selected through gap review. We may need more than two or three additional examples, particularly if the collection requires:

- stronger African, Asian, Latin American, or Indigenous coverage;
- more farmer-led and cooperative cases;
- more open agricultural software beyond data collection;
- more Global South open hardware, robotics, or sensing;
- more open data and biological-commons cases;
- cases that clarify what AI depends on upstream.

No additional quota is set at this stage.

Last reviewed: 2026-09-09 (two passes same day). A5 northern/remote pass, G-OSA-39: seven records — SIKU [sovereignty-first platform; ToS verified: no licence taken on user data; app code licence unverified], NWMB CBMN [harvester-owned data verified from NWMB FAQ + Arctic Science 2020; SMART/CyberTracker stack confirmed via SMART case study], SMART [open-source by claim, no public repo located — flagged], CyberTracker [MIT mobile client verified via GitHub API; CyberTracker Online hosting moved to tiered pricing 2025–26 with free access lane for Indigenous communities], CORE-POS [GPL-2.0 verified via GitHub API, active], Arctic Co-operatives Limited [governance record, curated with absence as finding; Li margin comparison sourced via Food Secure Canada op-ed], Harvest Food Hub [candidate — no public repo/licence found; correspondence with SSMIC flagged as the follow-up]. Collection now 128 records on disk. Prior same day, A4 cybersecurity/resilience pass, G-OSA-38: six records — JBS 2021, Sobeys/Empire 2022, Maple Leaf Foods 2022, NEW Cooperative 2021 [the collection's first incident records, curated as counter-examples], ISOBlue 2.0 [positive case — MIT verified via GitHub API across the ISOBlue org, local-storage design read from isoblue.org/docs], CSA T201 [verified against the SCC notice of intent: SDO comment 2026-08-18–2026-09-09]; collection 121 records. Prior state 115 records (2026-09-07, upcycling sub-cell, G-OSA-37: Pioreactor — the only licensed open fermentation-hardware platform, MIT software + CC BY-SA 4.0 hardware, Canadian; Sharing Excess — archived as the open-claim-without-licence case, GitHub API licence:null on both public repos; Second Harvest Food Rescue App — closed and commercially licensed out by a charity; FoodMesh/Mesh Exchange — closed B Corp infrastructure holding Metro Vancouver's public food-recovery contract; Upcycled Certified — certification capture of the category's definition, $175/product/year for the mark; LOOP Mission — closed Canadian upcycling flagship on proprietary HPP capital). Prior state 110 records (labour layer, G-OSA-36: WeClock — the corpus's first worker-held-data record; Digger's Mirth — the farmworker-cooperative anchor). Prior state 108 records (seven fisheries/aquaculture additions from G-OSA-34: Global Fishing Watch, sdmTMB, SS3, OpenCTD, Monitorfish, DFO aquaculture/sea-lice reporting, Skipper Otto; plus 2026-09-02's processing additions) (four grocery-surfaced additions: Foodsoft, FoodCoopShop, Foodsharing.de — retail co-op/rescue software layer; CSA K100:26 — Canadian FLW measurement standard; six processing-cell additions from G-OSA-33: IGFC mobile slaughter unit, OpenEPCIS, Underground Meats CC0 HACCP plan [archived — the failed-infrastructure case], Grain Bikes, ICAR ADE, CGC grading data) across software, data, standards, governance, hardware, machine data, DPI, China, Global South, cooperatives, biological commons, compost/soil-production open-knowledge (G-OSA-23), water/irrigation automation (G-OSA-25), livestock/pasture systems (G-OSA-26), post-harvest/cold-chain systems (G-OSA-27), cooperative logistics (G-OSA-28), open agroforestry canopy systems (G-OSA-29), open-science field spectroscopy/diagnostics (G-OSA-30), Chinese open IoT/LLM systems (G-LL-01 / G-OSA-19), Nepal community/digital seed and local-manufacturing leads (G-OSA-32), plant-scale processing (G-OSA-33), fisheries/aquaculture (G-OSA-34), land (G-OSA-35), upcycling/valorization (G-OSA-37), cybersecurity/resilience (G-OSA-38), and northern/remote food systems (G-OSA-39).

Not legal advice.
